...
#ray loop check
execute if score -distance ray_distance matches 1.. positioned ^ ^ ^.5 run function ut:move/snh_slash/ray

execute if score -distance ray_distance matches ..0 align yxz positioned ~0.5 ~ ~0.5 if entity @s[tag=targeting] if block ~ ~ ~ #ut:ray_trans if block ~ ~1 ~ #ut:ray_trans run function ut:move/snh_slash/ray_target