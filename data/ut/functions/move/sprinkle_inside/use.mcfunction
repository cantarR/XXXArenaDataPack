...
execute at @s unless entity @a[tag=effect_noise_despair] run execute as @a[tag=playing,tag=!respawning,tag=!untargetable,tag=!sprinkler,sort=nearest] unless score @s tid = #player tid if score #target pid matches 0 run function ut:move/sprinkle_inside/target
