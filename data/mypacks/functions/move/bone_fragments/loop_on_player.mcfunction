...
execute as @a if score @s pid = #atker sid run tag @s add target_player
execute at @p[tag=target_player,tag=playing,tag=!untargetable] run tp @s ~ ~0.5 ~
execute if score @s life matches 20 as @a[tag=target_player] run function ut:move/hit/main
execute if score @s life matches 40 as @a[tag=target_player] run function ut:move/hit/main
tag @s remove atker
