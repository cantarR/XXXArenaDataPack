...
execute if block ~ ~-0.1 ~ #ut:trans positioned ~ ~-0.1 ~ if score -store plt3b matches 1.. run function mypacks:move/library_of_ruina/snapping/loop_l_use3
execute unless entity @e[tag=lor_target_pos_use] unless block ~ ~-1 ~ #ut:trans positioned ~ ~ ~ run summon marker ~ ~1 ~ {Tags:[lor_target_pos_use]}
